...
class Tourcategory extends CI_Controller
{
    ...
    public function save_record()
    {
        ...
        $this->form_validation->set_rules('sub_category','Tour Category','trim|required|max_length[128]');
        ...
        if( $id == "" )
        {
            if( $this->tourcategory_model->checkRecordExists($sub_category) )
            {
                $response["error"] = 1;
        ...
        {
            $result = $this->tourcategory_model->checkRecordExists1($sub_category, $id);
            if( $result )
            {
                $response["error"] = 1;
                $response["error_message"] = "Record already exists";
